...
supabase/
  functions/
    ...
    generate-wallpaper/ # Background job: GPT → image gen → Storage → LINE push
    ...
    _shared/
      ...
      lineService.ts       # LINE reply/push/quickReply/paymentButtonMessage helpers
      ...
      db/
        ...
        userSessions.ts    # getActiveSession, createSession, updateSession, getSessionsForReminder, getGhostSessions
  ...
  migrations/
    0000001_schema.sql     # All table definitions
    0000002_seed.sql       # Initial prompts + pricing data
...
| Done but not delivered | `status=done`, `delivered_at IS NULL`, `completed_at < now-5min` | `product.deliver(lineUserId, order)` |
...
- Count 1–5: soft redirect | 6–8: direct redirect | 9+: guided mode (quick reply buttons) | 10+: deactivate session
...
### MCP Servers
Configured in `.mcp.json` — see file for details.

### AI CLI Tools
AI CLI tools (e.g. `codex`, `claude`, `gemini`) are available for development assistance.
