...
LINE OA Chatbot "น้องมงคล" — คุยธรรมชาติเหมือนคนจริง เก็บข้อมูล 7 อย่าง → จ่าย Stripe 159 บาท → ได้รูปมงคล + คำทำนาย + คาถา อัตโนมัติ ไม่มี admin
...
| Status           | Description                          |
| :--------------- | :----------------------------------- |
| collecting       | กำลังคุยกับน้องมงคล (GPT เก็บข้อมูล) |
| awaiting_payment | เก็บข้อมูลครบ รอ Stripe webhook       |
| done             | ส่งรูปและคำทำนายแล้ว                  |
...
| :---------------------------------- | :------------------------ | :-------------------------------------------- |
| กำลังคุย (status=collecting, ยังไม่มี order) | inactive > inactive_hours | Reminder (max 1 ครั้ง)                        |
| reminder ครบแล้ว                             | ไม่ตอบสนอง                | Deactivate (abandoned_reason = 'no_response') |
| รอจ่าย (status=awaiting_payment)             | Stripe link หมดอายุ       | stripe-webhook ส่ง link ใหม่อัตโนมัติ         |
...
_UNIQUE INDEX: (line_user_id) WHERE is_active = TRUE — active session ได้แค่ 1 ต่อ user_
...
_admin แก้ content ใน Supabase Dashboard ได้เลย ไม่ต้อง redeploy_
...
│   │   │   ├── handlers/
│   │   │   │   ├── message.ts   # entry point — consent → keyword → chat routing
│   │   │   │   ├── consent.ts   # PDPA consent flow
│   │   │   │   ├── keywords.ts  # special keyword handlers
│   │   │   │   └── chat.ts      # GPT conversation + order creation
│   │   │   └── lib/
│   │   │       ├── checkout.ts  # getOrRefreshCheckoutUrl
│   │   │       ├── guided.ts    # buildGuidedQuickReplies, extractedToCollected
│   │   │       └── messages.ts  # buildStatusMessage, buildMyDataMessage, etc.
...
│   │       ├── logger.ts            # logCtx() — structured log context prefix
